...
def test_config_settings():
    ...
    assert DEFAULT_MODEL.startswith("llama3.2"), f"Expected default model starting with llama3.2, got {DEFAULT_MODEL}"
    assert "11434" in DEFAULT_OLLAMA_HOST
    assert PAGE_CHAT in PAGES
    assert PAGE_DASHBOARD in PAGES
    assert len(PAGES) in [8, 9]

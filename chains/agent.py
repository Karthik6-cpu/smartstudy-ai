...
class StudyAgent:
    ...
    def detect_intent_fallback(self, query: str) -> Optional[Tuple[str, Dict[str, Any]]]:
        ...
        # 1. Math calculation pattern (e.g. 'What is 20 added to 30?', 'Calculate 25% of 480', '50 * 12', 'sqrt(144)')
        added_match = re.search(r"(\d+(?:\.\d+)?)\s+added\s+to\s+(\d+(?:\.\d+)?)", q_lower)
        if added_match:
            expr = f"{added_match.group(1)} + {added_match.group(2)}"
            return "calculate", {"expression": expr}

        calc_match = re.search(r"(?:calculate|compute|what is|solve|evaluate)\s+([0-9\.\s\+\-\*\/\%\^\(\)]+(?:of\s+[0-9\.]+)?|sqrt\([^)]+\))", q_lower)
        if calc_match:
            expr = calc_match.group(1).strip().rstrip("?.!;, ")
            return "calculate", {"expression": expr}
        elif any(c in q_lower for c in ["% of", "sqrt(", " + ", " - ", " * ", " / "]) and any(char.isdigit() for char in q_lower):
            clean_expr = re.sub(r"^(?:calculate|what is|compute|solve|evaluate)\s*", "", query, flags=re.IGNORECASE).strip().rstrip("?.!;, ")
            return "calculate", {"expression": clean_expr}

        # 2. PDF Summarization pattern (e.g. 'Summarize this PDF', 'Summarize my notes', 'Summarize it', 'I uploaded my notes. Summarize them.')
        ...
        # 3. PDF Quiz / Exam Questions pattern (e.g. 'Give me 10 questions from this PDF', 'Give me 5 MCQs from these notes', 'exam questions from this')
        ...
        # 4. Uploaded notes / PDF search pattern (e.g. 'from my uploaded notes', 'what does the PDF say about CMMI?', 'according to these notes', 'in these notes')
        ...
        # 5. Quiz request pattern (e.g. 'Give me 5 MCQs about DBMS joins', 'Quiz on DSA')
        ...
        # 6. Save memory pattern (e.g. 'Remember that my target exam score is 90%')
        ...
        # 7. Retrieve memory pattern (e.g. 'What is my target exam score?', 'What do you remember about me?')
        ...
        # 8. Study plan pattern (e.g. 'Create a 30-day study plan for TCS NQT', 'make me a study plan')
        ...
        # 9. MCA syllabus / curriculum overview pattern
